...
import { SeedOrdering } from '@/model/index.js';
import { OrderingMap } from './types.js';
...
export const ordering: OrderingMap = {
    ...
    inner_outer: <T>(array: T[]) => {
        ...
        while (positions.length < participantCount) {
            ...
            const next: number[] = [];
            for (const pos of positions) next.push(pos, size + 1 - pos);
            positions = next;
        }

        const result: T[] = [];
        for (const pos of positions) result.push(array[pos - 1]);
    ...
    'groups.seed_optimized': <T>(array: T[], groupCount: number) => {
        ...
        for (let run = 0; run < array.length / groupCount; run++) {
            if (run % 2 === 0) {
                ...
                for (let group = 0; group < groupCount; group++) {
                    groups[groupCount - group - 1].push(
                        array[run * groupCount + group],
                    );
                }
            }
        }
