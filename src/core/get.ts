...
export class Get extends BaseGetter {
        ...
        switch (stage.type) {
            case 'round_robin': {
                if (!roundRobinOptions) {
                    throw Error(
                        'Round-robin options are required for round-robin stages.',
                    );
                }
            ...
            case 'single_elimination': {
                if (roundRobinOptions) {
                    throw Error(
                        'Round-robin options are not supported for elimination stages.',
                    );
                }
     ...
     */
    private async roundRobinStandings(
        stage: Stage,
        roundRobinOptions: RoundRobinFinalStandingsOptions,
        ...
        const unsortedRanking = matchesByGroup.flatMap((groupMatches) => {
            const groupRanking = helpers.getRanking(
                groupMatches,
                roundRobinOptions.rankingFormula,
            );
            const qualifiedOnly = groupRanking.slice(
                0,
                roundRobinOptions.maxQualifiedParticipantsPerGroup,
            );
    ...
    private async singleEliminationStandings(
        ...
        if (stage.settings?.consolationFinal) {
            ...
            grouped.splice(
                2,
                1,
                [consolationFinalWinner],
                [consolationFinalLoser],
            );
        }
    ...
    private async doubleEliminationStandings(
        ...
        if (stage.settings?.grandFinal === 'none') {
            ...
            grouped[0] = [
                helpers.findParticipant(
                    participants,
                    getFinalWinnerIfDefined(finalWB),
                ),
            ];
            ...
            grouped[1] = [
                helpers.findParticipant(
                    participants,
                    getFinalWinnerIfDefined(finalLB),
                ),
            ];
        } else {
