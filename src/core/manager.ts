...
export class BracketsManager {
    ...
    constructor(storageInterface: CrudInterface, verbose?: boolean) {
            ...
            if (assertUnique && results.length > 1) {
                throw Error(
                    `Selecting ${JSON.stringify(
                        filter,
                    )} on table "${table}" must return a unique value.`,
                );
            }
        ...
        const createStageFunction = create.stage.bind(create);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        const createTournamentFunction = create.tournament.bind(create);
        this.create = Object.assign(createStageFunction, {
            stage: createStageFunction,
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            tournament: createTournamentFunction,
        }) as CallableCreate;
    ...
    public async import(data: Database, normalizeIds = false): Promise<void> {
        ...
        // Tournament is optional for backwards compatibility
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const tournaments = data.tournament;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        if (tournaments && tournaments.length > 0) {
            if (!(await this.storage.delete('tournament')))
                throw Error('Could not empty the tournament table.');
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-explicit-any
            if (!(await this.storage.insert('tournament', tournaments)))
                throw Error('Could not import tournaments.');
        }
    ...
    public async export(): Promise<Database> {
        ...
        try {
            const result = await this.storage.select('tournament');
            tournaments = result ?? undefined;
        } catch {
            // Storage doesn't support tournament table - this is fine
        ...
        if (tournaments) result.tournament = tournaments;
    ...
    private instrumentStorage(): void {
        ...
        const instrumentedMethods: Array<keyof CrudInterface> = [
            'insert',
            'select',
            'update',
            'delete',
        ...
        for (const method of Object.getOwnPropertyNames(
            ...
            if (!(instrumentedMethods as string[]).includes(method)) continue;
            ...
            storage[method] = async (
                ...
                if (verbose) {
                    ...
                    start = Date.now();
                    console.log(
                        `${id} ${method.toUpperCase()} "${table}" args: ${JSON.stringify(
                            args,
                        )}`,
                    );
                }
                ...
                if (verbose) {
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
                    console.log(
                        `${id!} ${duration}ms - Returned ${JSON.stringify(
                            result,
                        )}`,
                    );
                }
