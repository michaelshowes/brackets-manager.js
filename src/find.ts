...
export class Find extends BaseGetter {
    ...
    public async nextMatches(
        ...
        if (participantId !== undefined) {
            ...
            if (!helpers.isMatchStale(match)) {
                throw Error(
                    'The match is not stale yet, so it is not possible to conclude the next matches for this participant.',
                );
            }
            ...
            if (stage.type === 'double_elimination') {
                ...
                const {
                    ...
                    loserBracketMatch,
                    finalGroupMatch,
                } = await this.getMatchesByGroupDoubleElimination(
                    nextMatches,
                ...
                if (matchLocation === 'loser_bracket') {
                    if (participantId === loser?.id) return []; // Eliminated from lower bracket.
                    ...
                    if (!loserBracketMatch) {
                        throw Error(
                            'All matches of winner bracket should lead to loser bracket.',
                        );
                    }

                    if (participantId === loser?.id) return [loserBracketMatch]; // Eliminated from upper bracket, going to lower bracket.
                    ...
                    if (!finalGroupMatch) {
                        throw Error(
                            'All matches of a final group should also lead to the final group.',
                        );
                    }
     ...
     */
    private async getMatchesByGroupDoubleElimination(
        matches: Match[],
        ...
        for (const match of matches) {
            ...
            matchByGroupType = {
                winnerBracketMatch:
                    matchByGroupType['winnerBracketMatch'] ??
                    (helpers.isWinnerBracket('double_elimination', group.number)
                        ? match
                        : undefined),
                loserBracketMatch:
                    matchByGroupType['loserBracketMatch'] ??
                    (helpers.isLoserBracket('double_elimination', group.number)
                        ? match
                        : undefined),
                finalGroupMatch:
                    matchByGroupType['finalGroupMatch'] ??
                    (helpers.isFinalGroup('double_elimination', group.number)
                        ? match
                        : undefined),
            };
        }
